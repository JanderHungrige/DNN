...
#    pfad='/home/310122653/Git/DNN/Results/before15.11/'
#    pfad='/home/310122653/Git/DNN/Results/3fold/'
...
if fold>1:
       ...
       meank1=[np.mean([a,b,c],0),np.std([a,b,c],0)]
       meank2=[np.mean([a,c],0),np.std([a,c],0)]
       meank3=[np.mean([a,b],0),np.std([a,b],0)]
       meank4=[np.mean([b,c],0),np.std([b,c],0)]     
...
#              val,=plt.plot(XAxse,Ergebnisse.val_metric[i],label='Val')#,color=[0,0,0])
              val,=plt.plot(XAxse,Ergebnisse.val_k[i],label=str(i))
              plt.legend()
              plt.xlabel('Epochs')
              plt.ylabel('Kappa')
...
#---------1336
       standardt=np.std(Ergebnisse.train_k,0)
       standardv=whichone[1]         
       fig=plt.figure()
       ax=fig.add_subplot(gs[:-1,:])       
       ax.errorbar(XAxse,Ergebnisse.mean_train_k,standardt,errorevery=100,ecolor=[0.4,0.4,0.4],capsize=5,label='Train',color=[0.4,0.4,0.4])
       ax.errorbar(XAxse,whichone[0],standardv,errorevery=100,ecolor=[0,0,0],capsize=5,solid_capstyle='projecting',label='Val',color=[0,0,0])
       ax.legend(loc=0)
       ax.set_ylabel('Kappa') 
       ax.tick_params(axis='x',bottom='off',labelbottom='off') #only use one x axis       
